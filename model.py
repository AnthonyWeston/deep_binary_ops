...
class Model:
    ...
    def __init__(self, filename_list: list, training_size: int, batch_size: int, seed: int,
        ...
        self.learning_rate = tf.train.exponential_decay(self.initial_learning_rate, global_step = 
            self.learning_rate_global_step, decay_steps = 25 * self.training_size // self.batch_size, 
        ...
        self.hidden_layers = self._build_hidden_layers(self.inputs)
        ...
        self.prediction_accuracy = tf.reduce_mean(tf.to_float(tf.equal(self.predictions, self.z)),
            name = 'Accuracy')
